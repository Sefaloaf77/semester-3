...
if(isset($_SESSION['id_buku'])){
    ...
    while($data_f = mysqli_fetch_row($query_f)){
     ...
     if(empty($id_kategori_buku)){
	    header("Location:edit-buku-data-".$id_buku."_notif-editkosong-data-kategoribuku");
	}else if(empty($judul)){
	    header("Location:edit-buku-data-".$id_buku."_notif-editkosong-data-judul");
	}else if(empty($pengarang)){
	    header("Location:edit-buku-data-".$id_buku."_notif-editkosong-data-pengarang");
	}else if(empty($id_penerbit)){
	    header("Location:edit-buku-data-".$id_buku."_notif-editkosong-data-penerbit");
	}else if(empty($tahun_terbit)){
	    header("Location:edit-buku-data-".$id_buku."_notif-editkosong-data-tahunterbit");
	}else if(empty($tag)){
	    header("Location:edit-buku-data-".$id_buku."_notif-editkosong-data-tag");
	}else{
         $lokasi_file = $_FILES['cover']['tmp_name'];
	   $nama_file = $_FILES['cover']['name'];
	   $direktori = 'cover/'.$nama_file;
...
header("Location:buku_notif-editberhasil");
}
}
